...
test('VisaChat chat surface follows reference transfer and quick-reply interactions without internal notes', () => {
  ...
  for(const token of ['role="tablist"','role="tab"','role="tabpanel"','aria-selected={tab === id}','aria-expanded={open}','aria-controls={panelId}','handleTabKeyDown','requestAnimationFrame(() => notificationButtonRef.current?.focus())']) assert.ok(panel.includes(token),`settings accessibility contract missing ${token}`);
  for(const token of ['teamMembers.map','userOptions(option.defaultAssignee)','userOptions(draft.supervisor_user_id)','userOptions(draft.manager_user_id)','userOptions(rule.recipientUserId)']) assert.ok(panel.includes(token),`settings active-user selection missing ${token}`);
  assert.equal(panel.includes('placeholder="ID do usuário'),false,'VisaChat settings must not ask operators to type internal user ids');
  for(const token of ['aria-controls="visachat-notifications"','aria-controls="visachat-transfer-dialog"','aria-controls="visachat-quick-replies"','aria-controls="visachat-new-conversation"','requestAnimationFrame(() => newConversationTriggerRef.current?.focus())','openNewConversation(event.currentTarget)','teamMemberById.get(option.defaultAssignee']) assert.ok(app.includes(token),`VisaChat surface accessibility or assignee resolution missing ${token}`);
