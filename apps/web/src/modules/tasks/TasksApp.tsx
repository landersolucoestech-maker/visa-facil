...
function TaskModal({ mode, task, onClose, onSave }: { mode: ModalMode; task?: TaskRecord; onClose: () => void; onSave: (draft: TaskDraft) => void }) {
  ...
  if (mode === 'view' && task) {
    return <div className="tasks-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="tasks-view-modal" role="dialog" aria-modal="true">
      <header><div><span>TAREFA</span><h2>{task.title}</h2><p>{task.relatedType} · {task.relatedName || 'Sem vínculo'}</p></div><button type="button" onClick={onClose} aria-label="Fechar">×</button></header>
  ...
  return <div className="tasks-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="tasks-form-modal" role="dialog" aria-modal="true">
    <header><div><span>{mode === 'create' ? 'NOVA TAREFA' : 'EDITAR TAREFA'}</span><h2>{mode === 'create' ? 'Criar tarefa' : 'Editar tarefa'}</h2><p>Defina a atividade, o responsável e o prazo.</p></div><button type="button" onClick={onClose} aria-label="Fechar">×</button></header>
