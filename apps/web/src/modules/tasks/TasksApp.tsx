...
import './tasks.css';
import { taskArea, type RelatedType, type TaskArea, type TaskPriority, type TaskRecord, type TaskStatus } from './mocks/tasksMockProvider';
...
const AREA_OPTIONS:TaskArea[]=['Geral','Marketing'];
...
const EMPTY_DRAFT: TaskDraft = {
  title: '', description: '', relatedType: 'Contato', relatedName: '', relatedRecordId: '', owner: '', ownerUserId: '', area:'Geral', priority: 'Média', status: 'Pendente', dueDate: '', dueTime: '', reminder: 'Sem lembrete',
};
...
function SearchIcon() { return <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="6"/><path d="m16 16 4 4"/></svg>; }
...
function TaskModal({ mode, task, crmRecords, members, fixedArea, onClose, onSave }: { mode: ModalMode; task?: TaskRecord; crmRecords:CrmRecord[]; members:OperationalTeamMember[]; fixedArea?:TaskArea; onClose: () => void; onSave: (draft: TaskDraft) => void }) {
  const [draft, setDraft] = useState<TaskDraft>(() => task ? {
    title: task.title, description: task.description, relatedType: task.relatedType, relatedName: task.relatedName, relatedRecordId: task.relatedRecordId ?? '', owner: task.owner, ownerUserId: task.ownerUserId ?? '', area:fixedArea??taskArea(task),
  ...
  } : {...EMPTY_DRAFT,area:fixedArea??'Geral'});
  ...
  if (mode === 'view' && task) {
    return <div className="tasks-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="tasks-view-modal" role="dialog" aria-modal="true" aria-labelledby="tasks-view-title">
      <header><div><span>TAREFA</span><h2 id="tasks-view-title">{task.title}</h2><p>{taskArea(task)} · {task.relatedType} · {task.relatedName || 'Sem vínculo'}</p></div><button type="button" onClick={onClose} aria-label="Fechar">×</button></header>
      <section className="tasks-view-summary"><div><span>Área</span><strong>{taskArea(task)}</strong></div><div><span>Status</span><strong>{task.status}</strong></div><div><span>Prioridade</span><strong>{task.priority}</strong></div><div><span>Prazo</span><strong>{formatDate(task.dueDate)} {task.dueTime}</strong></div><div><span>Responsável</span><strong>{task.owner || 'Não atribuído'}</strong></div></section>
  ...
  return <div className="tasks-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="tasks-form-modal" role="dialog" aria-modal="true" aria-labelledby="tasks-form-title">
    <header><div><span>{mode === 'create' ? 'NOVA TAREFA' : 'EDITAR TAREFA'}</span><h2 id="tasks-form-title">{mode === 'create' ? 'Criar tarefa' : 'Editar tarefa'}</h2><p>Defina a atividade, a área, o responsável e o prazo.</p></div><button type="button" onClick={onClose} aria-label="Fechar">×</button></header>
    <form onSubmit={(event) => { event.preventDefault(); if (!invalid) onSave({...draft,area:fixedArea??draft.area??'Geral'}); }}><div className="tasks-form-grid">
      ...
      <label><span>Área</span><select value={fixedArea??draft.area??'Geral'} disabled={Boolean(fixedArea)} onChange={(event)=>set('area',event.target.value as TaskArea)}>{AREA_OPTIONS.map(option=><option key={option}>{option}</option>)}</select></label>
      ...
      <label><span>Horário</span><input type="time" disabled={!draft.dueDate} value={draft.dueTime} onChange={(event) => set('dueTime', event.target.value)} /></label>
      <label><span>Lembrete</span><select disabled={!draft.dueDate} value={draft.reminder} onChange={(event) => set('reminder', event.target.value)}>{REMINDER_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select></label>
    ...
    </div><footer><button type="button" className="crm-btn-secondary" onClick={onClose}>Cancelar</button><button type="submit" className="crm-btn-primary" disabled={invalid}>Salvar tarefa</button></footer></form>
  </div></div>;
}

export function TasksApp({fixedArea}:{fixedArea?:TaskArea}={}) {
  const [tasks, setTasks] = useState<TaskRecord[]>(() => getTaskSessionRecords());
  const [query, setQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState('Todos');
  const [priorityFilter, setPriorityFilter] = useState('Todas');
  const [areaFilter,setAreaFilter]=useState<'Todas'|TaskArea>(fixedArea??'Todas');
  ...
  const scopedTasks=useMemo(()=>fixedArea?tasks.filter(task=>taskArea(task)===fixedArea):tasks,[tasks,fixedArea]);
  const filtered = useMemo(() => scopedTasks.filter((task) => {
    const haystack = `${task.title} ${task.relatedName} ${task.owner} ${task.description} ${taskArea(task)}`.toLowerCase();
    return haystack.includes(query.trim().toLowerCase()) && (statusFilter === 'Todos' || task.status === statusFilter) && (priorityFilter === 'Todas' || task.priority === priorityFilter) && (fixedArea||areaFilter==='Todas'||taskArea(task)===areaFilter);
  }), [scopedTasks, query, statusFilter, priorityFilter, areaFilter, fixedArea]);

  const saveTask = (draft: TaskDraft) => {
    const now = new Date().toISOString();
    const normalizedDraft:TaskDraft={...draft,area:fixedArea??draft.area??'Geral'};
    if (modal?.task) setTasks((current) => current.map((task) => task.id === modal.task?.id ? { ...task, ...normalizedDraft, updatedAt: now } : task));
    else setTasks((current) => [{ ...normalizedDraft, id: crypto.randomUUID(), createdAt: now, updatedAt: now }, ...current]);
    setModal(undefined);
  };
  const deleteTask = (task: TaskRecord) => { if (window.confirm(`Excluir a tarefa “${task.title}”?`)) setTasks((current) => current.filter((item) => item.id !== task.id)); };
  const stats = {
    total: scopedTasks.length,
    pending: scopedTasks.filter((task) => task.status !== 'Concluída').length,
    today: scopedTasks.filter((task) => task.dueDate === today && task.status !== 'Concluída').length,
    overdue: scopedTasks.filter((task) => task.dueDate && task.dueDate < today && task.status !== 'Concluída').length,
  };
  const notificationCount = stats.overdue + stats.today;
  const marketing=fixedArea==='Marketing';

  return <div className="tasks-shell" onClick={() => { setOpenActionId(undefined); setNotificationsOpen(false); }} onKeyDown={(event)=>{if(event.key==='Escape'){setOpenActionId(undefined);setNotificationsOpen(false);if(modal)setModal(undefined)}}}>
    <div className="crm-workspace tasks-workspace">
      <header className="crm-topbar tasks-topbar"><div><small>{marketing?'VISA FÁCIL · CRM · MARKETING':'VISA FÁCIL · CRM'}</small><h1>{marketing?'Tarefas de Marketing':'Tarefas'}</h1><p>{marketing?'Atividades e prazos do fluxo de marketing usando a mesma base canônica de tarefas do CRM.':'Atividades, prazos e acompanhamentos operacionais.'}</p></div><div className="crm-topbar-actions tasks-topbar-actions" onClick={(event) => event.stopPropagation()}>
