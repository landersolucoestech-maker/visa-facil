...
import { communicationProvidersByClass } from '../integrations/communicationContract';
...
const TELECOM_PROVIDERS=communicationProvidersByClass('telecom-carrier').map(provider=>provider.name).join(' · ');
const CLOUD_COMMUNICATION_PROVIDERS=communicationProvidersByClass('cloud-communications').map(provider=>provider.name).join(' · ');
...
export function IntegrationsTab(){
 ...
 return <Card title="Integrações" description="Conectores externos configuráveis pela operação" icon="↗">
  ...
  {categories.map(category=><div className="settings-integration-category" key={category}><div className="settings-integration-category-title"><span>◈</span><strong>{category}</strong></div>{INTEGRATION_REGISTRY.filter(item=>item.category===category).map(item=>{const status=statusFor(item.id);const working=busy===item.id;const linked=status.state==='connected'||status.state==='degraded'||Boolean(status.accountId||status.accountLabel);const authorized=status.authorizedCapabilities?.map(capability=>CAPABILITY_LABELS[capability]);const planned=item.capabilities.map(capability=>CAPABILITY_LABELS[capability]);return <div className="settings-integration-row" key={item.id}><div className="settings-integration-logo">{ICONS[item.id]}</div><div className="settings-integration-copy"><strong>{item.name}</strong><p>{item.description}</p>{item.officialAuthorizationProvider&&<small>Autorização: fluxo oficial {AUTH_PROVIDER_LABEL[item.officialAuthorizationProvider]}</small>}{item.id==='meta'&&<><small>Produtos / canais: Facebook · Instagram · Messenger · Meta Ads</small><small>Ativos previstos: Páginas do Facebook · Contas profissionais do Instagram · Business Portfolio · Contas de anúncios</small><small>Meta App ID, Meta App Secret, OAuth, tokens, webhook, Graph API e estado geral pertencem ao provider Meta e não são duplicados entre Facebook e Instagram.</small>{status.metaProducts?.length?<div className="settings-meta-products" aria-label="Produtos conectados ao provider Meta">{META_PRODUCTS.map(product=>{const productStatus=status.metaProducts?.find(value=>value.id===product.id);return <div className="settings-meta-product" key={product.id}><span>{product.name}</span><b>{productStatus?META_PRODUCT_STATUS_LABEL[productStatus.state]:'Não informado pelo backend'}</b>{productStatus?.assetIds?.length?<small>{productStatus.assetIds.length} ativo{productStatus.assetIds.length===1?'':'s'} vinculado{productStatus.assetIds.length===1?'':'s'}</small>:null}</div>})}</div>:<small>Estados por produto: {backendConfigured&&linked?'não informados pelo backend':backendConfigured?'disponíveis após autorização do provider Meta':'Backend necessário'}</small>}{status.metaAssets?.length?<small>Ativos autorizados informados pelo backend: {status.metaAssets.length}</small>:null}</>}{item.id==='telephony-sms'&&<><small>Operadoras tradicionais previstas: {TELECOM_PROVIDERS}</small><small>Provedores IP/Internet previstos: {CLOUD_COMMUNICATION_PROVIDERS}</small><small>Credenciais, linhas/números, remetentes, rotas e webhooks serão configurados por provider adapter no backend; disponibilidade depende do contrato e das APIs do fornecedor escolhido.</small></>}{status.accountLabel&&<small>Conta vinculada: {status.accountLabel}</small>}{status.accountId&&<small>ID da conta: {status.accountId}</small>}{linked?<small>Recursos autorizados: {authorized?.length?authorized.join(' · '):'não informados pelo backend'}</small>:<small>Recursos previstos: {item.id==='meta'?'Facebook · Instagram · Messenger · Mensagens · Atendimento · Informações da conta · Páginas · Publicação · Gestão de conteúdo · Comentários e moderação · Campanhas e anúncios · Métricas e analytics':planned.join(' · ')}</small>}{item.authMode==='oauth2'&&linked&&<small>Permissões concedidas: {status.grantedScopes?.length?status.grantedScopes.join(' · '):'não informadas pelo backend'}</small>}{status.lastSyncAt&&<small>Última sincronização: {formatDateTime(status.lastSyncAt)}</small>}{status.expiresAt&&<small>Autorização válida até: {formatDateTime(status.expiresAt)}</small>}{status.errorMessage&&<small role="alert">{status.errorMessage}</small>}</div><div className="settings-integration-actions"><span className={statusClass(status.state)}>{statusLabel(item.id,status.state)}</span>{linked?<><button className="settings-btn settings-btn-outline" type="button" disabled={!backendConfigured||working} onClick={()=>void run(item.id,'sync')}>{working?'Processando…':'Sincronizar'}</button>{item.authMode==='oauth2'&&<button className="settings-btn settings-btn-outline" type="button" disabled={!backendConfigured||working} onClick={()=>void run(item.id,'reconnect')}>Reconectar</button>}<button className="settings-btn settings-btn-outline" type="button" disabled={!backendConfigured||working} onClick={()=>void run(item.id,'disconnect')}>Desconectar</button></>:<button className="settings-btn settings-btn-outline" type="button" disabled={!backendConfigured||working} onClick={()=>void run(item.id,'connect')}>{working?'Processando…':backendConfigured?(item.authMode==='oauth2'?`Conectar ${item.name}`:`Configurar ${item.name}`):'Backend necessário'}</button>}</div></div>})}</div>)}
 </Card>;
}
