import { AUTHENTICATION_ENABLED, getAuthSession, signOut } from '../auth/auth';
...
export function WorkspaceSelectorApp(){
 ...
 return <main className="workspace-page">
  <header className="workspace-header"><a className="workspace-brand" href={`${basePath()}/`}><BrandMark/><span><strong>VISA FÁCIL</strong><small>Plataforma interna</small></span></a><div className="workspace-account"><div><strong>{session?.name||'Administrador'}</strong><small>{AUTHENTICATION_ENABLED?(session?.email||'Conta interna'):'Autenticação desativada'}</small></div>{AUTHENTICATION_ENABLED&&<button onClick={()=>{signOut();go('/login')}}>Sair</button>}</div></header>
