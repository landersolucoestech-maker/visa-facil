...
const EMPTY_DRAFT: RecordDraft = {
  ...
  personType: 'Pessoa Física', company: '', role: '', relationship: 'Cliente', contactStatus: 'Ativo', source: 'Website', owner: '',
  interest: '', destination: '', visaType: '', leadStatus: 'Novo', temperature: 'Morno', nextAction: '', nextActionDate: '',
};
...
function RecordForm({ kind, initial, onCancel, onSubmit }: { kind: RecordKind; initial: RecordDraft; onCancel: () => void; onSubmit: (draft: RecordDraft) => void }) {
  ...
  return <form className="crm-record-form" onSubmit={(event) => { event.preventDefault(); if (!draft.fullName.trim() || !draft.email.trim()) return; onSubmit(draft); }}>
    <div className="crm-form-section"><div className="crm-form-section__heading"><strong>Identificação</strong><small>Dados principais do {kind === 'contact' ? 'contato' : 'lead'}.</small></div><div className="crm-form-grid">
      ...
      {kind === 'contact' && <><Field label="Tipo de contato"><select value={draft.personType} onChange={(e) => set('personType', e.target.value)}><option>Pessoa Física</option><option>Pessoa Jurídica</option></select></Field><Field label="Empresa"><input value={draft.company} onChange={(e) => set('company', e.target.value)} /></Field><Field label="Cargo / Função"><input value={draft.role} onChange={(e) => set('role', e.target.value)} /></Field><Field label="Relacionamento"><select value={draft.relationship} onChange={(e) => set('relationship', e.target.value)}><option>Cliente</option><option>Parceiro</option><option>Fornecedor</option><option>Prestador</option><option>Outro</option></select></Field></>}
      {kind === 'lead' && <><Field label="Origem"><select value={draft.source} onChange={(e) => set('source', e.target.value)}><option>Website</option><option>WhatsApp</option><option>Instagram</option><option>Facebook</option><option>Indicação</option><option>Google</option><option>Outro</option></select></Field><Field label="Interesse / Serviço"><input value={draft.interest} onChange={(e) => set('interest', e.target.value)} /></Field><Field label="Destino de interesse"><input value={draft.destination} onChange={(e) => set('destination', e.target.value)} /></Field><Field label="Tipo de visto / Interesse"><input value={draft.visaType} onChange={(e) => set('visaType', e.target.value)} /></Field></>}
    </div></div>
...
function RecordModal({ mode, kind, record, onClose, onSave, onEdit }: { mode: ModalMode; kind: RecordKind; record?: CrmRecord; onClose: () => void; onSave: (draft: RecordDraft) => void; onEdit: () => void }) {
  ...
  return <div className="crm-modal-backdrop" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><div className="crm-form-modal" role="dialog" aria-modal="true"><header><div><span>{mode === 'create' ? 'NOVO REGISTRO' : 'EDITAR REGISTRO'}</span><h2>{mode === 'create' ? 'Novo' : 'Editar'} {kind === 'contact' ? 'contato' : 'lead'}</h2><p>{mode === 'create' ? 'Preencha os dados para adicionar o registro ao CRM.' : 'Atualize os dados cadastrados no CRM.'}</p></div><button type="button" onClick={onClose} aria-label="Fechar">×</button></header><RecordForm kind={kind} initial={draft} onCancel={onClose} onSubmit={onSave} /></div></div>;
}
