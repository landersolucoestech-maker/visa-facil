...
function ServiceFields({ draft, set }: { draft: CrmRecordDraft; set: (key: keyof CrmRecordDraft, value: string) => void }) {
  return <>
    <Field label="Interesse / Serviço"><select value={draft.interest} onChange={(event) => set('interest', event.target.value)}><option value="">Selecione o serviço</option>{SERVICE_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select></Field>
    <Field label="Destino de interesse"><select value={draft.destination} onChange={(event) => set('destination', event.target.value)}><option value="">Selecione o destino</option>{DESTINATION_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select></Field>
    <Field label="Tipo de visto / Interesse"><select value={draft.visaType} onChange={(event) => set('visaType', event.target.value)}><option value="">Selecione o tipo de visto</option>{VISA_TYPE_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select></Field>
  </>;
}
...
function RecordForm({ kind, initial, onCancel, onSubmit }: { kind: RecordKind; initial: CrmRecordDraft; onCancel: () => void; onSubmit: (draft: CrmRecordDraft) => void }) {
  ...
  return <form className="crm-record-form" onSubmit={(event) => { event.preventDefault(); if (!draft.fullName.trim() || !draft.email.trim()) return; onSubmit(draft); }}>
    <div className="crm-form-section"><div className="crm-form-section__heading"><strong>Identificação pessoal</strong><small>Dados principais e documentos da pessoa atendida.</small></div><div className="crm-form-grid">
      ...
      {kind === 'contact' ? <Field label="Relacionamento"><select value={draft.relationship} onChange={(event) => set('relationship', event.target.value)}><option>Cliente</option><option>Parceiro</option><option>Outro</option></select></Field> : <Field label="Origem"><select value={draft.source} onChange={(event) => set('source', event.target.value)}><option>Website</option><option>WhatsApp</option><option>Instagram</option><option>Facebook</option><option>Indicação</option><option>Google</option><option>Outro</option></select></Field>}
    </div></div>
    ...
    <div className="crm-form-section"><div className="crm-form-section__heading"><strong>{kind === 'contact' ? 'Relacionamento' : 'Qualificação comercial'}</strong><small>Contexto operacional do registro.</small></div><div className="crm-form-grid">
      {kind === 'contact' ? <><Field label="Status"><select value={draft.contactStatus} onChange={(event) => set('contactStatus', event.target.value)}><option>Ativo</option><option>Inativo</option></select></Field><Field label="Origem do contato"><select value={draft.source} onChange={(event) => set('source', event.target.value)}><option>Website</option><option>WhatsApp</option><option>Instagram</option><option>Facebook</option><option>Indicação</option><option>Outro</option></select></Field><Field label="Responsável"><input value={draft.owner} onChange={(event) => set('owner', event.target.value)} /></Field></> : <><Field label="Status do lead"><select value={draft.leadStatus} onChange={(event) => set('leadStatus', event.target.value)}><option>Novo</option><option>Em contato</option><option>Qualificado</option><option>Não qualificado</option><option>Convertido</option><option>Perdido</option></select></Field><Field label="Temperatura"><select value={draft.temperature} onChange={(event) => set('temperature', event.target.value)}><option>Frio</option><option>Morno</option><option>Quente</option></select></Field><Field label="Responsável"><input value={draft.owner} onChange={(event) => set('owner', event.target.value)} /></Field><Field label="Próxima ação"><input value={draft.nextAction} onChange={(event) => set('nextAction', event.target.value)} /></Field><Field label="Data da próxima ação"><input type="date" value={draft.nextActionDate} onChange={(event) => set('nextActionDate', event.target.value)} /></Field></>}
