...
function RecordView({ record, onClose, onEdit, titleId }: { record: CrmRecord; onClose: () => void; onEdit: () => void; titleId: string }) {
...
function RecordModal({ mode, kind, record, onClose, onSave, onEdit }: { mode: ModalMode; kind: RecordKind; record?: CrmRecord; onClose: () => void; onSave: (draft: CrmRecordDraft) => void; onEdit: () => void }) {
  ...
  if (mode === 'view' && record) return <div className="crm-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="crm-view-modal" role="dialog" aria-modal="true" aria-labelledby={titleId}><RecordView record={record} onClose={onClose} onEdit={onEdit} titleId={titleId} /></div></div>;
