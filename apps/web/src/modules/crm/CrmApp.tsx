...
function ServiceFields({ draft, set }: { draft: RecordDraft; set: (key: keyof RecordDraft, value: string) => void }) { return <><Field label="Interesse / Serviço"><select value={draft.interest} onChange={(e) => set('interest', e.target.value)}><option value="">Selecione o serviço</option>{SERVICE_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></Field><Field label="Destino de interesse"><select value={draft.destination} onChange={(e) => set('destination', e.target.value)}><option value="">Selecione o destino</option>{DESTINATION_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></Field><Field label="Tipo de visto / Interesse"><select value={draft.visaType} onChange={(e) => set('visaType', e.target.value)}><option value="">Selecione o tipo de visto</option>{VISA_TYPE_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></Field></>; }
...
function RecordForm({ kind, initial, onCancel, onSubmit }: { kind: RecordKind; initial: RecordDraft; onCancel: () => void; onSubmit: (draft: RecordDraft) => void }) {
  ...
  return <form className="crm-record-form" onSubmit={(event) => { event.preventDefault(); if (!draft.fullName.trim() || !draft.email.trim()) return; onSubmit(draft); }}>
    <div className="crm-form-section"><div className="crm-form-section__heading"><strong>Identificação pessoal</strong><small>Dados principais e documentos da pessoa atendida.</small></div><div className="crm-form-grid"><Field label="Nome completo"><input required value={draft.fullName} onChange={(e) => set('fullName', e.target.value)} placeholder="Nome completo" /></Field><Field label="CPF"><input value={draft.cpf} onChange={(e) => set('cpf', e.target.value)} placeholder="000.000.000-00" /></Field><Field label="RG"><input value={draft.rg} onChange={(e) => set('rg', e.target.value)} placeholder="Número do RG" /></Field><Field label="Número do passaporte"><input value={draft.passportNumber} onChange={(e) => set('passportNumber', e.target.value)} placeholder="Número do passaporte" /></Field><ServiceFields draft={draft} set={set} />{kind === 'contact' ? <Field label="Relacionamento"><select value={draft.relationship} onChange={(e) => set('relationship', e.target.value)}><option>Cliente</option><option>Parceiro</option><option>Outro</option></select></Field> : <Field label="Origem"><select value={draft.source} onChange={(e) => set('source', e.target.value)}><option>Website</option><option>WhatsApp</option><option>Instagram</option><option>Facebook</option><option>Indicação</option><option>Google</option><option>Outro</option></select></Field>}</div></div>
