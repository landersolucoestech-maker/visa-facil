...
function ServiceFields({ draft, set }: { draft: RecordDraft; set: (key: keyof RecordDraft, value: string) => void }) {
  return <>
...
function RecordForm({ kind, initial, onCancel, onSubmit }: { kind: RecordKind; initial: RecordDraft; onCancel: () => void; onSubmit: (draft: RecordDraft) => void }) {
  ...
  return <form className="crm-record-form" onSubmit={(event) => { event.preventDefault(); if (!draft.fullName.trim() || !draft.email.trim()) return; onSubmit(draft); }}>
    <div className="crm-form-section"><div className="crm-form-section__heading"><strong>Identificação pessoal</strong><small>Dados principais e documentos da pessoa atendida.</small></div><div className="crm-form-grid">
      <Field label="Nome completo"><input required value={draft.fullName} onChange={(e) => set('fullName', e.target.value)} placeholder="Nome completo" /></Field>
    ...
    <div className="crm-form-section"><div className="crm-form-section__heading"><strong>{kind === 'contact' ? 'Relacionamento' : 'Qualificação comercial'}</strong><small>Contexto operacional.</small></div><div className="crm-form-grid">
      ...
      <Field label="Observações"><textarea rows={4} value={draft.notes} onChange={(e) => set('notes', e.target.value)} placeholder="Observações importantes..." /></Field>
    </div></div>
...
function RecordView({ record, onClose, onEdit }: { record: CrmRecord; onClose: () => void; onEdit: () => void }) {
  ...
  return <div className="crm-view-record">
    ...
    <section className="crm-view-section"><div className="crm-view-section__title"><span>03</span><div><strong>{record.kind === 'contact' ? 'Relacionamento e interesse' : 'Qualificação'}</strong><small>Contexto operacional do registro.</small></div></div><div className="crm-view-grid">{record.kind === 'contact' ? <><DetailItem label="Relacionamento" value={record.relationship} /><DetailItem label="Interesse / Serviço" value={record.interest} /><DetailItem label="Destino de interesse" value={record.destination} /><DetailItem label="Tipo de visto / Interesse" value={record.visaType} /><DetailItem label="Origem" value={record.source} /><DetailItem label="Responsável" value={record.owner} /></> : <><DetailItem label="Origem" value={record.source} /><DetailItem label="Interesse / Serviço" value={record.interest} /><DetailItem label="Destino de interesse" value={record.destination} /><DetailItem label="Tipo de visto / Interesse" value={record.visaType} /><DetailItem label="Temperatura" value={record.temperature} /></>}</div></section>
