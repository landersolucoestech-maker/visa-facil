...
import { CmsStorageError, loadDraft, publishDraft, saveDraft } from './siteStore';
...
export function SiteCmsApp(){
 ...
 const publish=()=>{try{const saved=saveDraft(document);const next=publishDraft(saved);setDocumentState(next);setDirty(false);showNotice('Conteúdo publicado localmente neste navegador.',3000)}catch(error){showNotice(storageMessage(error),4200)}};
 const preview=()=>{try{saveDraft(document);const slug=selectedPage?.slug||'/';window.open(`${href('/preview')}?cmsPreview=draft&page=${encodeURIComponent(slug)}`,'_blank','noopener,noreferrer')}catch(error){showNotice(storageMessage(error),4200)}};
