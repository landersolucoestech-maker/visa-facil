...
import { isSafeCmsExternalUrl } from './cmsDocumentContract';
...
export function FieldEditor({field,value,onChange,media}:{field:CmsFieldDefinition;value:CmsValue|undefined;onChange:(value:CmsValue)=>void;media:CmsMediaItem[]}){
 ...
 if(field.type==='image'){const current=typeof value==='string'?value:'';const preview=cmsImageSrc(current);return <label className="site-cms-field"><span>{field.label}</span>{field.help&&<small>{field.help}</small>}<div className="site-cms-image-input"><input value={current} onChange={e=>onChange(e.target.value)} placeholder="URL da imagem"/><select value="" onChange={e=>e.target.value&&onChange(e.target.value)}><option value="">Escolher da mídia…</option>{media.filter(asset=>asset.kind==='image').map(asset=><option value={asset.url} key={asset.id}>{asset.name}</option>)}</select></div>{preview&&<img className="site-cms-image-preview" src={preview} alt="Prévia"/>}{current&&!preview&&<small className="site-cms-form-error" role="alert">Imagem inválida ou não suportada.</small>}</label>}
...
export function SeoEditor({page,onChange,media}:{page:CmsPage;onChange:(page:CmsPage)=>void;media:CmsMediaItem[]}){
 const seo=page.seo;const set=<K extends keyof typeof seo>(key:K,value:(typeof seo)[K])=>onChange({...page,seo:{...seo,[key]:value},updatedAt:now()});const canonicalError=seo.canonicalUrl.trim()&&!isSafeCmsExternalUrl(seo.canonicalUrl)?'A Canonical URL deve usar http:// ou https://.':'';const ogImageError=seo.ogImage.trim()&&!isSafeCmsExternalUrl(seo.ogImage)?'A OG Image deve usar uma URL absoluta http:// ou https://.':'';
 return <div className="site-cms-editor"><div className="site-cms-editor-heading"><div><span>SEO DA PÁGINA</span><h2>Metadados</h2><p>Configurações utilizadas por buscadores e compartilhamentos.</p></div></div><div className="site-cms-form-grid"><label><span>Title</span><input value={seo.title} onChange={e=>set('title',e.target.value)}/><small>{seo.title.length}/60 caracteres</small></label><label className="is-wide"><span>Meta description</span><textarea rows={4} value={seo.description} onChange={e=>set('description',e.target.value)}/><small>{seo.description.length}/160 caracteres</small></label><label><span>OG Image</span><div className="site-cms-image-input"><input value={seo.ogImage} onChange={e=>set('ogImage',e.target.value)} placeholder="https://…"/><select value="" onChange={e=>e.target.value&&set('ogImage',e.target.value)}><option value="">Escolher mídia externa…</option>{media.filter(asset=>asset.kind==='image'&&isSafeCmsExternalUrl(asset.url)).map(asset=><option value={asset.url} key={asset.id}>{asset.name}</option>)}</select></div>{ogImageError&&<small className="site-cms-form-error" role="alert">{ogImageError}</small>}</label><label><span>Canonical URL</span><input type="url" value={seo.canonicalUrl} onChange={e=>set('canonicalUrl',e.target.value)}/>{canonicalError&&<small className="site-cms-form-error" role="alert">{canonicalError}</small>}</label><label className="site-cms-checkbox"><input type="checkbox" checked={seo.noIndex} onChange={e=>set('noIndex',e.target.checked)}/><span>Impedir indexação desta página</span></label></div></div>;
}