...
function isCanonicalClient(record:CrmRecord){return record.kind==='contact'&&record.relationship==='Cliente'}
...
export function ContractEditorModal({record,contacts,templates,variables,onClose,onSave}:{record?:ContractRecord;contacts:CrmRecord[];templates:ContractTemplate[];variables:ContractVariableDefinition[];onClose:()=>void;onSave:(draft:ContractEditorDraft,template:ContractTemplate,documentContent:string)=>void}){
 ...
 const canonicalClients=contacts.filter(isCanonicalClient);
 const selectedClient=contacts.find(item=>item.id===draft.clientId);
 const legacyLinkedClient=draft.clientId&&!canonicalClients.some(item=>item.id===draft.clientId)?selectedClient:undefined;
 const clientSourceValue=draft.clientId?(canonicalClients.some(item=>item.id===draft.clientId)?draft.clientId:'legacy'):'manual';
 ...
 const chooseClient=(id:string)=>{
  ...
  const contact=canonicalClients.find(item=>item.id===id);if(!contact)return;
  ...
  <section className="contracts-editor" role="dialog" aria-modal="true" aria-labelledby="contracts-editor-title">
   ...
   <div className="contracts-editor-main">
    ...
    <div className="contracts-editor-body">
     ...
     {step===1&&<div className="contracts-form-section"><div className="contracts-section-copy"><h4>Contratante / cliente</h4><p>Vincule um cliente canônico do CRM ou use entrada manual. Leads precisam ser convertidos em Cliente no CRM antes de serem vinculados ao contrato. O contrato preserva um snapshot dos dados selecionados.</p></div><label className="contracts-field contracts-field--wide"><span>Origem da parte</span><select value={clientSourceValue} onChange={event=>chooseClient(event.target.value)}><option value="manual">Preenchimento manual</option>{legacyLinkedClient&&<option value="legacy">{legacyLinkedClient.fullName} · vínculo legado/indisponível</option>}{canonicalClients.map(contact=><option key={contact.id} value={contact.id}>{contact.fullName} · Cliente CRM</option>)}</select></label>{legacyLinkedClient&&<div className="contracts-warning"><strong>Vínculo CRM legado.</strong><p>Este contrato aponta para um registro que não é mais um Cliente canônico. O vínculo foi preservado; selecione um Cliente CRM para relincar ou mantenha o snapshot histórico.</p></div>}<div className="contracts-form-grid"><TextField label="Nome completo" value={clientParty.name} onChange={value=>setParty({name:value})} readOnly={clientParty.source==='crm'}/><TextField label="CPF" value={clientParty.cpf} onChange={value=>setParty({cpf:value})} readOnly={clientParty.source==='crm'}/><TextField label="RG" value={clientParty.rg} onChange={value=>setParty({rg:value})} readOnly={clientParty.source==='crm'}/><TextField label="Passaporte" value={clientParty.passportNumber} onChange={value=>setParty({passportNumber:value})} readOnly={clientParty.source==='crm'}/><TextField label="E-mail" value={clientParty.email} onChange={value=>setParty({email:value})} type="email" readOnly={clientParty.source==='crm'}/><TextField label="Telefone" value={clientParty.phone} onChange={value=>setParty({phone:value})} readOnly={clientParty.source==='crm'}/></div></div>}
