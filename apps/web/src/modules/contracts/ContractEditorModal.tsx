...
import type { ContractCategory, ContractEditorDraft, ContractParty, ContractRecord, ContractSigner, ContractTemplate, ContractVariableDefinition } from './contractTypes';
...
export function ContractEditorModal({record,contacts,templates,categories,variables,onClose,onSave}:{record?:ContractRecord;contacts:CrmRecord[];templates:ContractTemplate[];categories:ContractCategory[];variables:ContractVariableDefinition[];onClose:()=>void;onSave:(draft:ContractEditorDraft,template:ContractTemplate,documentContent:string)=>void}){
 ...
 const chooseTemplate=(id:string)=>{
  ...
  setDraft(current=>({...current,templateId:id,categoryId:template.categoryId||current.categoryId,title:current.title||template.name}));
 };
 ...
 const stepValid=[Boolean(draft.templateId),Boolean(clientParty.name.trim()),Boolean(draft.title.trim()&&draft.categoryId&&draft.startDate&&draft.serviceDescription.trim()&&requiredCustomComplete),true,signersValid,true][step]??false;
 const canSave=Boolean(selectedTemplate&&draft.title.trim()&&draft.categoryId&&draft.startDate&&draft.serviceDescription.trim()&&clientParty.name.trim());
  ...
  <section className="contracts-editor" role="dialog" aria-modal="true" aria-labelledby="contracts-editor-title">
   ...
   <div className="contracts-editor-main">
    ...
    <div className="contracts-editor-body">
     {step===0&&<div className="contracts-template-picker"><p>Selecione um modelo. O documento será preenchido pelos dados do CRM e pelas variáveis do contrato.</p>{activeTemplates.length?activeTemplates.map(template=><button type="button" key={template.id} className={draft.templateId===template.id?'is-selected':''} onClick={()=>chooseTemplate(template.id)}><div><strong>{template.name}</strong><span>{categories.find(item=>item.id===template.categoryId)?.label??'Sem categoria'}</span><p>{template.description}</p></div><b>{draft.templateId===template.id?'Selecionado':'Selecionar'}</b></button>):<div className="contracts-empty-inline">Nenhum template ativo. Crie um template antes de cadastrar contratos.</div>}</div>}
     ...
     {step===5&&<div className="contracts-review"><div className="contracts-section-copy"><h4>Revisão final</h4><p>Revise os dados antes de salvar. “Salvar para revisão” não envia o documento ao Autentique.</p></div><div className="contracts-review-grid"><div><span>Template</span><strong>{selectedTemplate?.name??'—'}</strong></div><div><span>Cliente</span><strong>{clientParty.name||'—'}</strong></div><div><span>Categoria</span><strong>{categories.find(item=>item.id===draft.categoryId)?.label??'—'}</strong></div><div><span>Signatários</span><strong>{draft.signers.length}</strong></div><div><span>Destino</span><strong>{draft.destination||'—'}</strong></div><div><span>Tipo de visto</span><strong>{draft.visaType||'—'}</strong></div></div><ContractDocumentPreview content={documentContent}/>{unresolved.length>0&&<div className="contracts-warning"><strong>Documento incompleto.</strong><p>Preencha as variáveis pendentes antes de considerar o documento pronto para assinatura.</p></div>}</div>}
    </div>
