...
export function ClientTable({ clients }: ClientTableProps) {
  ...
  return <div className="management-table-wrap"><table className="management-table"><thead><tr><th>Cliente</th><th>Contato</th><th>Status</th><th>Criado em</th><th>Ação</th></tr></thead><tbody>{clients.map((client) => <tr key={client.id}><td><strong>{client.fullName}</strong>{client.notes && <small>{client.notes}</small>}</td><td><span>{client.phone}</span><small>{client.email}</small></td><td><span className={`management-badge management-badge--${client.status}`}>{CLIENT_STATUS_LABELS[client.status]}</span></td><td>{new Date(client.createdAt).toLocaleDateString('pt-BR')}</td><td><a className="management-inline-link" href={`/app/clientes/${encodeURIComponent(client.id)}`}>Abrir cliente →</a></td></tr>)}</tbody></table></div>;
}
