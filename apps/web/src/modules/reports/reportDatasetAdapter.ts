...
function importTasks(rows:ReportRow[]):ImportResult{
 ...
 rows.forEach((row,index)=>{
  const rowNumber=index+2;const title=required(row,'Título',rowNumber);const relatedType=canonicalOption(text(row,'Tipo de vínculo'),TASK_RELATED_TYPES,'Contato','Tipo de vínculo',rowNumber) as RelatedType;const relatedName=text(row,'Contato / Lead relacionado');const priority=canonicalOption(text(row,'Prioridade'),TASK_PRIORITIES,'Média','Prioridade',rowNumber) as TaskPriority;const status=canonicalOption(text(row,'Status'),TASK_STATUSES,'Pendente','Status',rowNumber) as TaskStatus;const dueDate=excelDate(text(row,'Data'),'Data',rowNumber,false);const dueTime=dueDate?excelTime(text(row,'Horário'),'Horário',rowNumber):'';const reminder=dueDate?canonicalOption(text(row,'Lembrete'),TASK_REMINDERS,'Sem lembrete','Lembrete',rowNumber):'Sem lembrete';if(!dueDate&&(text(row,'Horário')||text(row,'Lembrete')))throw new Error(`Linha ${rowNumber}: Horário e Lembrete exigem uma Data.`);const owner=text(row,'Responsável')||'Administrador';const description=text(row,'Descrição');
  const existingIndex=next.findIndex(record=>normalize(record.title)===normalize(title)&&record.relatedType===relatedType&&normalize(record.relatedName)===normalize(relatedName)&&record.dueDate===dueDate&&record.dueTime===dueTime);const previous=existingIndex>=0?next[existingIndex]:undefined;const timestamp=nowIso();const record:TaskRecord={id:previous?.id??newId('task'),title,description,relatedType,relatedName,owner,priority,status,dueDate,dueTime,reminder,createdAt:previous?.createdAt??timestamp,updatedAt:timestamp};if(existingIndex>=0){next[existingIndex]=record;updated+=1}else{next.push(record);imported+=1}
 });
