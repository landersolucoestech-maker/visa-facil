...
import { cmsHref, cmsImageSrc, cmsList, cmsTarget, cmsText, itemText, usePageSection } from '../content/SiteContentContext';
...
export function HeroSection({sectionId='hero'}:{sectionId?:string}) {
  ...
  const primaryTarget=cmsTarget(cmsText(values.primaryCtaTarget,'_self'));const secondaryTarget=cmsTarget(cmsText(values.secondaryCtaTarget,'_self'));
  return (
    <section className="hero" aria-label="Destaques VISA FÁCIL">
      <div className="hero-slider hero-slider--background" data-hero-slider="" role="region" aria-roledescription="carousel" aria-label="Destaques e campanhas">
        {slides.map((slide,index)=><article className={`hero-slide${index===0?' is-active':''}`} data-hero-slide="" role="group" aria-roledescription="slide" aria-label={`${index+1} de ${slides.length}`} aria-hidden={index!==0} key={`${itemText(slide,'image')}-${index}`}><img src={cmsImageSrc(itemText(slide,'image'),heroVisaFacil)} alt={itemText(slide,'alt','Destaque VISA FÁCIL')}/></article>)}
      ...
      <div className="container hero__grid"><div className="hero__content reveal"><h1>{cmsText(values.headlineLine1,'O caminho mais fácil')}<br/>{cmsText(values.headlineLine2,'para o seu visto')}<br/><em>{cmsText(values.headlineAccent,'começa aqui.')}</em></h1><div className="hero__trust">{trust.map((item,index)=><div key={index}><span className="check">✓</span><b>{itemText(item,'text')}</b></div>)}</div><div className="hero__mini-benefits" aria-label="Diferenciais do atendimento">{mini.map((item,index)=><div key={index}><span>{itemText(item,'icon','•')}</span><b>{itemText(item,'text')}</b></div>)}</div><div className="hero__actions"><a className="btn btn--primary btn--large" href={cmsHref(cmsText(values.primaryCtaUrl),'#diagnostico')} target={primaryTarget} rel={primaryTarget==='_blank'?'noreferrer':undefined}>{cmsText(values.primaryCtaLabel,'Analisar meu perfil')}</a><a className="btn btn--outline btn--large" href={cmsHref(cmsText(values.secondaryCtaUrl),'#servicos')} target={secondaryTarget} rel={secondaryTarget==='_blank'?'noreferrer':undefined}>{cmsText(values.secondaryCtaLabel,'Conhecer os serviços')}</a></div></div></div>
