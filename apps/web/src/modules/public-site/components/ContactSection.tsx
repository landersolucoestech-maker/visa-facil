...
import { safePublicFieldName, safePublicFieldType } from '../content/publicFormSafety';
...
export function ContactSection({sectionId='contact'}:{sectionId?:string}) {
  ...
  const safeFields=fields.map((item,index)=>({item,name:safePublicFieldName(itemText(item,'name'),index,usedNames),type:safePublicFieldType(itemText(item,'type','text'))}));
