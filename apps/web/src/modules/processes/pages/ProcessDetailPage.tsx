...
export function ProcessDetailPage({ process, client, documents, interactions, tasks, financialEntries, onUpdateProcess }: ProcessDetailPageProps) {
  ...
  return <section className="management-page process-detail-page" aria-labelledby="process-detail-title">
    ...
    <section className="process-update-card" aria-labelledby="process-update-title"><div><span className="management-eyebrow">Atualização operacional</span><h2 id="process-update-title">Evoluir processo</h2><p>Atualize os campos operacionais já previstos no contrato. As alterações permanecem somente nesta sessão.</p></div><label><span>Etapa</span><select value={process.stage} onChange={(event) => onUpdateProcess(process.id, { stage: event.target.value as ProcessStage })}>{Object.entries(PROCESS_STAGE_LABELS).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span>Prioridade</span><select value={process.priority} onChange={(event) => onUpdateProcess(process.id, { priority: event.target.value as ProcessPriority })}>{Object.entries(PROCESS_PRIORITY_LABELS).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span>Data-alvo</span><input type="date" value={process.targetDate} onChange={(event) => onUpdateProcess(process.id, { targetDate: event.target.value })} /></label></section>

    <section className="process-stage-timeline" aria-label="Linha do tempo do processo"><div className="process-stage-timeline__heading"><span className="management-eyebrow">Jornada do processo</span><strong>{process.stage === 'cancelled' ? 'Processo cancelado' : `${stageIndex + 1} de ${operationalStages.length} etapas`}</strong></div><div className="process-stage-timeline__track">{operationalStages.map((stage,index)=>{const state=process.stage==='cancelled'?'is-muted':index<stageIndex?'is-complete':index===stageIndex?'is-current':'';return <div className={`process-stage-timeline__step ${state}`} key={stage}><span>{index<stageIndex?'✓':index+1}</span><strong>{PROCESS_STAGE_LABELS[stage]}</strong></div>;})}</div></section>
    ...
    <div className="process-detail-grid">
      <section className="process-detail-card"><span className="management-eyebrow">Dados do processo</span><h2>Visão geral</h2><dl className="process-detail-data"><div><dt>Destino</dt><dd>{DESTINATION_LABELS[process.destination]}</dd></div><div><dt>Categoria</dt><dd>{process.category}</dd></div><div><dt>Prioridade</dt><dd>{PROCESS_PRIORITY_LABELS[process.priority]}</dd></div><div><dt>Data-alvo</dt><dd>{process.targetDate ? new Date(`${process.targetDate}T00:00:00`).toLocaleDateString('pt-BR') : 'Sem data definida'}</dd></div><div><dt>Atualizado em</dt><dd>{new Date(process.updatedAt).toLocaleString('pt-BR')}</dd></div></dl><p className="process-detail-notes">{process.notes || 'Nenhuma observação registrada.'}</p></section>
