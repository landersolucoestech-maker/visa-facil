...
function write<T>(key: string, value: T) { sessionStorage.setItem(key, JSON.stringify(value)); }
...
export function applyFinanceRules(record: FinanceRecord): FinanceRecord {
  const categories = activeFinanceCategories(record.type);
  const categoryNames = new Set(categories.map((category) => category.name));
  const normalizedDescription = record.description.toLocaleUpperCase('pt-BR');
  const rule = getFinanceRules().find((candidate) => candidate.active && candidate.type === record.type && categoryNames.has(candidate.category) && normalizedDescription.includes(candidate.contains.trim().toLocaleUpperCase('pt-BR')));
