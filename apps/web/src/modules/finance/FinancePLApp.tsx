...
export function FinancePLApp() {
  ...
  const entries = useMemo<Entry[]>(() => records
    ...
    .map((record) => ({
      id: record.id,
      kind: record.type,
      category: record.category || (record.type === 'Receita' ? 'Receita' : 'Despesa operacional'),
  ...
  const alerts = useMemo<AccountingAlert[]>(() => {
    ...
    else if (!filtered.length) items.push({ id: 'empty-period', title: 'Sem movimentações', detail: 'Nenhuma transação reconhecida no período selecionado.', tone: 'neutral' });
    ...
    <div className="crm-workspace accounting-workspace-main">
      <header className="crm-topbar accounting-topbar">
        ...
        <div className="crm-topbar-actions accounting-topbar-actions" onClick={(event) => event.stopPropagation()}>
          ...
          <div className="accounting-topbar-menu"><button className="crm-user accounting-user" type="button" onClick={() => { setUser((current) => !current); setNotifications(false); }}><span>VF</span><div><strong>Administrador</strong><small>Protótipo frontend</small></div><span className="crm-user-caret">⌄</span></button>{user && <div className="accounting-dropdown accounting-user-dropdown"><button type="button">Perfil</button><a href={href('/crm/configuracoes')}>Configurações</a><button className="is-danger" type="button">Logout</button></div>}</div>
        </div>
      </header>
