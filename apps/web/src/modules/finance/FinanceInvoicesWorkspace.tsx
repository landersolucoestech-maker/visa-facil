  ...
  return <div className="finance-accounting-backdrop" onMouseDown={(event) => event.currentTarget === event.target && close()}>
    <div className="finance-invoice-form invoice-refined-form">
      ...
      <form onSubmit={(event) => { event.preventDefault(); save(draft); }}>
        ...
        <Section title="Valores"><Field label="Serviço"><input type="number" step="0.01" value={draft.serviceFee || ''} onChange={(event) => set('serviceFee', Number(event.target.value))} /></Field><Field label="Taxa consular"><input type="number" step="0.01" value={draft.consularFee || ''} onChange={(event) => set('consularFee', Number(event.target.value))} /></Field><Field label="Tradução"><input type="number" step="0.01" value={draft.translationFee || ''} onChange={(event) => set('translationFee', Number(event.target.value))} /></Field><Field label="Terceiros"><input type="number" step="0.01" value={draft.thirdPartyFee || ''} onChange={(event) => set('thirdPartyFee', Number(event.target.value))} /></Field><Field label="Outros valores"><input type="number" step="0.01" value={draft.otherCharges || ''} onChange={(event) => set('otherCharges', Number(event.target.value))} /></Field><Field label="Descontos"><input type="number" step="0.01" value={draft.discounts || ''} onChange={(event) => set('discounts', Number(event.target.value))} /></Field></Section>
        <Section title="Tributos"><Field label="ICMS"><input type="number" step="0.01" value={draft.icms || ''} onChange={(event) => set('icms', Number(event.target.value))} /></Field><Field label="IPI"><input type="number" step="0.01" value={draft.ipi || ''} onChange={(event) => set('ipi', Number(event.target.value))} /></Field><Field label="PIS"><input type="number" step="0.01" value={draft.pis || ''} onChange={(event) => set('pis', Number(event.target.value))} /></Field><Field label="COFINS"><input type="number" step="0.01" value={draft.cofins || ''} onChange={(event) => set('cofins', Number(event.target.value))} /></Field><Field label="ISS"><input type="number" step="0.01" value={draft.iss || ''} onChange={(event) => set('iss', Number(event.target.value))} /></Field><Field label="Tributos retidos"><input type="number" step="0.01" value={draft.withheldTaxes || ''} onChange={(event) => set('withheldTaxes', Number(event.target.value))} /></Field></Section>
        ...
        <footer><button type="button" className="crm-btn-secondary" onClick={close}>Cancelar</button><button type="submit" className="crm-btn-primary">Salvar invoice</button></footer>
      </form>
    </div>
  </div>;
}
...
export function FinanceInvoicesWorkspace() {
  ...
  const pay = (payment: Payment) => {
    ...
    setItems((current) => current.map((invoice) => {
      ...
      const nextPaid = Math.min(total(invoice), invoice.paid + payment.amount);
      return { ...invoice, paid: nextPaid, payments: [payment, ...invoice.payments], status: nextPaid >= total(invoice) ? 'Pago' : 'Parcialmente pago' };
    }));
    setModal(undefined);
  };
        ...
        <section className="invoice-list-card">
          ...
          <div className="invoice-table-scroll"><div className="invoice-table">
            ...
            {rows.length ? rows.map((invoice) => { const currentStatus = visibleStatus(invoice); return <div className="invoice-table-row" key={invoice.id}>
              ...
              <div className="invoice-balance-cell" data-label="Saldo"><strong className={balance(invoice) > 0 ? 'is-outstanding' : ''}>{money(balance(invoice))}</strong><small>{invoice.paid > 0 ? `${money(invoice.paid)} recebido` : 'Sem pagamentos'}</small></div>
