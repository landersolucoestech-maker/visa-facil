...
const EMPTY_NEW_CONVERSATION: NewConversationDraft = { customer: '', handle: '', channel: 'WhatsApp', message: '', participantIds: [] };
const EMPTY_TEAM_CONVERSATION: NewConversationDraft = { customer: '', handle: '', channel: 'Equipe', message: '', participantIds: [] };
...
export function AttendanceApp() {
  ...
  const currentAuthor = teamMembers.find((member) => member.role === 'Administrador')?.name ?? 'Administrador';
  ...
  }), [modeConversations, mode, query, statusFilter, teamMemberById]);
  ...
  const sendMessage = () => {
    ...
    const nextMessage: AttendanceMessage = { id: crypto.randomUUID(), sender: 'agent', author: currentAuthor, body, time: timeNow() };
    const updatedAt = new Date().toISOString();
  ...
  const openNewConversation = () => {
    ...
    setNewConversation(mode === 'team'
      ? { ...EMPTY_TEAM_CONVERSATION, participantIds: teamMembers.map((member) => member.id) }
  ...
  const createConversation = () => {
    ...
    if (newConversationKind === 'team') {
      ...
      if (newConversation.participantIds.length === 0) { setNewConversationError('Selecione pelo menos um participante ativo.'); return; }
      const selectedMembers = teamMembers.filter((member) => newConversation.participantIds.includes(member.id));
      ...
      const initialMessage: AttendanceMessage[] = initialBody ? [{ id: crypto.randomUUID(), sender: 'agent', author: currentAuthor, body: initialBody, time: now }] : [];
      const record: AttendanceConversation = {
        id,
        kind: 'team',
        customer,
        handle: selectedMembers.map((member) => member.name).join(' · '),
        email: '',
        channel: 'Equipe',
        status: 'Ativo',
