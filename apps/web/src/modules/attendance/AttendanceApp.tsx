...
export function AttendanceApp() {
  ...
  if (settingsOpen) {
    return <div className="attendance-shell"><div className="crm-workspace attendance-workspace"><AttendanceSettingsPanel teamMembers={teamMembers} onClose={() => { setSettingsOpen(false); setSettingsRevision((value) => value + 1); }} /></div></div>;
  }
  ...
  return <div className="attendance-shell" onClick={() => setNotificationsOpen(false)}>
    <div className="crm-workspace attendance-workspace">
      <header className="crm-topbar attendance-topbar">
        <div><small>VISA FÁCIL · CRM</small><h1>VisaChat</h1><p>Atendimento multicanal e comunicação interna da equipe.</p></div>
        <div className="crm-topbar-actions attendance-topbar-actions" onClick={(event) => event.stopPropagation()}>
          <button className="attendance-settings-button" type="button" onClick={() => setSettingsOpen(true)}><SettingsIcon/><span>Configurações</span></button>
          <button className="crm-topbar-primary attendance-new-conversation" type="button" aria-label={mode === 'team' ? 'Novo chat interno' : 'Nova conversa'} onClick={openNewConversation}><PlusIcon/><span>{mode === 'team' ? 'Novo chat interno' : 'Nova conversa'}</span></button>
      ...
      <main className="attendance-content"><section className="attendance-inbox">
        <aside className="attendance-list-panel"><div className="attendance-list-header"><div><span>{mode === 'team' ? 'EQUIPE' : 'ATENDIMENTO'}</span><h2>{mode === 'team' ? 'Comunicação interna' : 'Inbox'}</h2></div><button type="button" aria-label={mode === 'team' ? 'Novo chat interno' : 'Nova conversa'} onClick={openNewConversation}><PlusIcon/></button></div>
          <div className="attendance-mode-tabs" role="tablist" aria-label="Tipo de conversa" onKeyDown={handleModeKeyDown}><button id="visachat-tab-customer" type="button" role="tab" aria-selected={mode === 'customer'} aria-controls="visachat-mode-panel" tabIndex={mode === 'customer' ? 0 : -1} className={mode === 'customer' ? 'is-active' : ''} onClick={() => changeMode('customer')}>Atendimento <small>{customerConversations.length}</small></button><button id="visachat-tab-team" type="button" role="tab" aria-selected={mode === 'team'} aria-controls="visachat-mode-panel" tabIndex={mode === 'team' ? 0 : -1} className={mode === 'team' ? 'is-active' : ''} onClick={() => changeMode('team')}>Equipe <small>{teamConversations.length}</small></button></div>
          <div className="attendance-mode-panel" id="visachat-mode-panel" role="tabpanel" aria-labelledby={`visachat-tab-${mode}`}>
            {mode === 'customer' ? <div className="attendance-support-nav" aria-label="Filas de atendimento">{customerQueues.map((queue) => <button key={queue} type="button" className={queueFilter === queue ? 'is-active' : ''} onClick={() => setQueueFilter(queue)}>{queue === 'Todos' ? 'Todos' : queue}</button>)}</div> : <div className="attendance-team-type-tabs" aria-label="Tipos de chat interno"><button type="button" className={teamTypeFilter === 'all' ? 'is-active' : ''} onClick={() => setTeamTypeFilter('all')}>Todos</button>{TEAM_CONVERSATION_TYPES.map((type) => <button key={type} type="button" className={teamTypeFilter === type ? 'is-active' : ''} onClick={() => setTeamTypeFilter(type)}>{teamTypeLabel(type)}</button>)}</div>}
            <div className={`attendance-filters ${mode === 'team' ? 'is-team' : ''}`}><label><SearchIcon/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={mode === 'team' ? 'Buscar na equipe' : 'Buscar atendimento'} aria-label={mode === 'team' ? 'Buscar chat interno' : 'Buscar conversa'} /></label>{mode === 'customer' && <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} aria-label="Filtrar conversas por status"><option>Todos</option>{CUSTOMER_STATUS_OPTIONS.map((status) => <option key={status}>{status}</option>)}</select>}</div>
            ...
            <div className="attendance-conversation-list">{filtered.length === 0 ? <p className="attendance-empty">{mode === 'team' ? 'Nenhum chat interno encontrado.' : 'Nenhuma conversa encontrada.'}</p> : filtered.map((conversation) => { const kind = getAttendanceConversationKind(conversation); const type = getAttendanceTeamType(conversation); return <button key={conversation.id} type="button" className={`${conversation.id === selected?.id ? 'is-active' : ''} ${kind === 'team' ? 'is-team' : ''}`.trim()} onClick={() => selectConversation(conversation)}><span className="attendance-avatar">{conversation.customer.replace(/^#/, '').slice(0, 2).toUpperCase()}</span><span className="attendance-conversation-copy"><span className="attendance-conversation-line"><strong>{conversation.customer}</strong><small>{conversation.lastMessageAt}</small></span><span className="attendance-meta"><b>{kind === 'team' ? teamTypeLabel(type ?? 'group') : conversation.channel}</b><em>{conversation.status}</em></span><span className="attendance-preview">{conversation.lastMessage}</span></span>{conversation.unread > 0 && <span className="attendance-unread">{conversation.unread}</span>}</button>; })}</div>
          </div>
        </aside>
    ...
    {newConversationOpen && <div className="attendance-modal-backdrop" onMouseDown={(event) => event.currentTarget === event.target && setNewConversationOpen(false)}><div className="attendance-modal" role="dialog" aria-modal="true" aria-labelledby="visachat-new-conversation-title"><header><div><span>{creatingTeamChat ? 'CHAT INTERNO' : 'NOVA CONVERSA'}</span><h2 id="visachat-new-conversation-title">{creatingTeamChat ? 'Novo espaço da equipe' : 'Iniciar conversa'}</h2><p>{creatingTeamChat ? 'Crie uma conversa direta, grupo ou canal usando somente usuários ativos de Configurações.' : 'O vínculo com canal externo continua local até a integração específica de mensagens existir.'}</p></div><button type="button" onClick={() => setNewConversationOpen(false)} aria-label="Fechar">×</button></header><div className="attendance-modal-body">{creatingTeamChat ? <><label><span>Tipo</span><select value={newConversation.teamType} onChange={(event) => setNewConversation((current) => ({ ...current, teamType: event.target.value as AttendanceTeamType, participantIds: [], customer: '' }))}><option value="direct">Conversa direta</option><option value="group">Grupo</option><option value="channel">Canal</option></select></label>{newConversation.teamType !== 'direct' && <label><span>{newConversation.teamType === 'channel' ? 'Nome do canal' : 'Nome do grupo'}</span><input value={newConversation.customer} onChange={(event) => setNewConversation((current) => ({ ...current, customer: event.target.value }))} placeholder={newConversation.teamType === 'channel' ? 'Ex.: geral, comercial ou financeiro' : 'Ex.: Diretoria ou Equipe Comercial'} /></label>}<fieldset className="attendance-team-members"><legend>{newConversation.teamType === 'direct' ? 'Usuário' : 'Participantes'}</legend>{selectableTeamMembers.length ? selectableTeamMembers.map((member) => <label key={member.id} className="attendance-team-member-option"><input type={newConversation.teamType === 'direct' ? 'radio' : 'checkbox'} name={newConversation.teamType === 'direct' ? 'direct-participant' : undefined} checked={newConversation.participantIds.includes(member.id)} onChange={() => newConversation.teamType === 'direct' ? setNewConversation((current) => ({ ...current, participantIds: [member.id] })) : toggleTeamParticipant(member.id)} /><span><strong>{member.name}</strong><small>{member.role} · {member.email}</small></span></label>) : <p className="attendance-team-help">Nenhum outro usuário ativo está disponível em Configurações → Usuários.</p>}</fieldset><p className="attendance-team-help">Você ({currentAuthor}) é incluído automaticamente. Nenhum número de telefone é necessário para comunicação interna.</p><label><span>Mensagem inicial</span><textarea rows={4} value={newConversation.message} onChange={(event) => setNewConversation((current) => ({ ...current, message: event.target.value }))} placeholder="Mensagem inicial opcional" /></label></> : <><label><span>Nome do contato / lead</span><input value={newConversation.customer} onChange={(event) => setNewConversation((current) => ({ ...current, customer: event.target.value }))} placeholder="Nome completo" /></label><label><span>Canal</span><select value={newConversation.channel} onChange={(event) => setNewConversation((current) => ({ ...current, channel: event.target.value }))}><option>WhatsApp</option><option>Instagram</option><option>Facebook</option><option>Website</option><option>E-mail</option></select></label><label><span>Telefone / usuário</span><input value={newConversation.handle} onChange={(event) => setNewConversation((current) => ({ ...current, handle: event.target.value }))} placeholder="Contato do canal" /></label><label><span>Mensagem inicial</span><textarea rows={4} value={newConversation.message} onChange={(event) => setNewConversation((current) => ({ ...current, message: event.target.value }))} placeholder="Mensagem inicial opcional" /></label><p className="attendance-team-help">A conversa será criada no VisaChat, mas mensagens externas só podem ser marcadas como entregues quando o canal estiver realmente integrado.</p></>}{newConversationError && <p className="attendance-empty" role="alert">{newConversationError}</p>}</div><footer><button className="crm-btn-secondary" type="button" onClick={() => setNewConversationOpen(false)}>Cancelar</button><button className="crm-btn-primary" type="button" onClick={createConversation}>{creatingTeamChat ? 'Criar' : 'Iniciar conversa'}</button></footer></div></div>}
  </div>;
}
