...
export function AttendanceSettingsPanel({ teamMembers, onClose }: Props) {
  ...
  const userOptions = (currentId?: string | null) => <><option value="">Não atribuído</option>{currentId && !teamMembers.some((member) => member.id === currentId) && <option value={currentId}>Usuário indisponível ({currentId})</option>}{teamMembers.map((member) => <option key={member.id} value={member.id}>{member.name} · {member.role}</option>)}</>;
  ...
  const handleTabKeyDown = (event: KeyboardEvent<HTMLElement>) => {
    ...
    event.preventDefault();
    const tabs: SettingsTab[] = ['messages', 'menu', 'escalation', 'templates'];
    const index = tabs.indexOf(tab);
    const next = event.key === 'Home' ? 0 : event.key === 'End' ? tabs.length - 1 : (index + (event.key === 'ArrowRight' ? 1 : -1) + tabs.length) % tabs.length;
    setTab(tabs[next]);
    requestAnimationFrame(() => document.getElementById(`visachat-settings-tab-${tabs[next]}`)?.focus());
  };
  ...
  return <div className="visachat-ref-screen" onClick={() => setNotificationsOpen(false)}>
    ...
    <main className="attendance-content visachat-ref-page" aria-label="Automações do VisaChat">
      ...
      {tab === 'escalation' && <div className="visachat-ref-stack" id="visachat-settings-panel-escalation" role="tabpanel" aria-labelledby="visachat-settings-tab-escalation">
        <article className="visachat-ref-card"><header><h2>Responsáveis padrão</h2></header><div className="visachat-ref-grid2"><label><span>Supervisor padrão</span><select value={draft.supervisor_user_id ?? ''} onChange={(e)=>patch({supervisor_user_id:e.target.value||null})}>{userOptions(draft.supervisor_user_id)}</select></label><label><span>Gestor padrão</span><select value={draft.manager_user_id ?? ''} onChange={(e)=>patch({manager_user_id:e.target.value||null})}>{userOptions(draft.manager_user_id)}</select></label></div></article>
        <article className="visachat-ref-card"><header><div><h2>Regras de escalonamento</h2><p>Defina o tempo-limite e o destino de cada nível. Estas regras são avaliadas quando o escalonamento é executado manualmente; não há disparo automático por temporizador nesta versão.</p></div><button type="button" className="crm-btn-primary" onClick={addEscalation}>+ Adicionar regra</button></header><div className="visachat-ref-accordion">{[...draft.escalation_rules].sort((a,b)=>a.afterMinutes-b.afterMinutes).map((rule)=>{const open=openEscalationIds[rule.id]??false;const panelId=`visachat-escalation-rule-${rule.id}`;return <div key={rule.id} className="visachat-ref-accordion-item"><button type="button" className="visachat-ref-accordion-trigger" aria-expanded={open} aria-controls={panelId} onClick={()=>setOpenEscalationIds((current)=>({...current,[rule.id]:!open}))}><span className="visachat-ref-number">{rule.afterMinutes} min</span><strong>{rule.level||'Sem nível'}</strong><small>Destino: {RECIPIENT_LABELS[rule.recipientRole]??rule.recipientRole}</small><em>{rule.active?'Ativa':'Inativa'}</em><b>{open?'⌄':'›'}</b></button>{open&&<div className="visachat-ref-accordion-content" id={panelId}><div className="visachat-ref-escalation-row"><label><span>Minutos</span><input type="number" min="1" value={rule.afterMinutes} onChange={(e)=>updateEscalation(rule.id,{afterMinutes:Number(e.target.value)||1})}/></label><label><span>Nível</span><input value={rule.level} onChange={(e)=>updateEscalation(rule.id,{level:e.target.value})}/></label><label><span>Destino</span><select value={rule.recipientRole} onChange={(e)=>updateEscalation(rule.id,{recipientRole:e.target.value,recipientUserId:e.target.value==='custom'?rule.recipientUserId:null})}><option value="supervisor">Supervisor</option><option value="manager">Gestor</option><option value="custom">Usuário específico</option></select></label>{rule.recipientRole==='custom'&&<label><span>Usuário específico</span><select value={rule.recipientUserId??''} onChange={(e)=>updateEscalation(rule.id,{recipientUserId:e.target.value||null})}>{userOptions(rule.recipientUserId)}</select></label>}<div className="visachat-ref-inline-actions"><Switch checked={rule.active} onChange={(active)=>updateEscalation(rule.id,{active})} label="Ativa"/><button type="button" className="visachat-ref-delete" onClick={()=>patch({escalation_rules:draft.escalation_rules.filter((item)=>item.id!==rule.id)})}>Excluir</button></div></div><div className="visachat-ref-channel-row">{VISACHAT_NOTIFICATION_CHANNELS.map((channel)=><label key={channel}><input type="checkbox" checked={rule.channels.includes(channel)} onChange={(e)=>toggleRuleChannel(rule,channel,e.target.checked)}/><span>{CHANNEL_LABELS[channel]}</span></label>)}</div></div>}</div>})}</div></article>
      </div>}

      {tab === 'templates' && <article className="visachat-ref-card" id="visachat-settings-panel-templates" role="tabpanel" aria-labelledby="visachat-settings-tab-templates"><header><div><h2>Questionários por serviço</h2><p>Abra um serviço do menu e configure quais perguntas serão coletadas inicialmente para ele.</p></div><button type="button" className="crm-btn-primary" onClick={addServiceQuestionnaire}>+ Adicionar questionário</button></header><div className="visachat-ref-questionnaires">{[...draft.menu_options].sort((a,b)=>a.order-b.order).map((option)=>{const open=openQuestionnaireIds[option.id]??false;const fields=option.required_fields?.length?option.required_fields:draft.required_fields;const template=getTemplate(option);const panelId=`visachat-questionnaire-${option.id}`;return <div key={option.id} className="visachat-ref-questionnaire"><button type="button" className="visachat-ref-questionnaire-trigger" aria-expanded={open} aria-controls={panelId} onClick={()=>setOpenQuestionnaireIds((current)=>({...current,[option.id]:!open}))}><div><strong>{option.order}. {option.label}</strong><span>{fields.length} campo(s)</span>{!option.active&&<span>Inativa</span>}<small>Fila {option.queue} / Setor {option.sector}</small></div><b>{open?'⌄':'›'}</b></button>{open&&<div className="visachat-ref-questionnaire-content" id={panelId}><div className="visachat-ref-questionnaire-actions"><button type="button" className="visachat-ref-delete" onClick={()=>removeServiceQuestionnaire(option.id)}>Excluir questionário</button></div><section className="visachat-ref-inner-card"><h3>Campos obrigatórios</h3><p>Perguntas que precisam ser respondidas para este tipo de serviço.</p><div className="visachat-ref-field-list">{fields.length?fields.map((field,index)=><div key={`${field}-${index}`}><input aria-label={`Campo obrigatório ${index+1} de ${option.label}`} value={field} onChange={(e)=>updateRequiredFields(option.id,fields.map((item,i)=>i===index?e.target.value:item))}/><button type="button" className="visachat-ref-delete" aria-label={`Excluir campo ${field || index+1}`} onClick={()=>updateRequiredFields(option.id,fields.filter((_,i)=>i!==index))}>Excluir</button></div>):<p>Nenhum campo obrigatório configurado para este serviço.</p>}</div><div className="visachat-ref-add-field"><input aria-label={`Novo campo obrigatório de ${option.label}`} value={newFields[option.id]??''} onChange={(e)=>setNewFields((current)=>({...current,[option.id]:e.target.value}))} onKeyDown={(e)=>{if(e.key==='Enter'){e.preventDefault();addField(option.id,fields)}}} placeholder="Ex.: Nome completo"/><button type="button" className="crm-btn-primary" onClick={()=>addField(option.id,fields)}>+ Adicionar</button></div></section><section className="visachat-ref-inner-card"><h3>Mensagem Automática</h3><textarea aria-label={`Mensagem automática de ${option.label}`} rows={4} value={template.body} onChange={(e)=>updateTemplateForOption(option,e.target.value)}/></section></div>}</div>})}</div></article>}
    </main>
  </div>;
}
