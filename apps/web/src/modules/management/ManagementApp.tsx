...
type InteractionUpdate = Partial<Pick<ServiceInteraction, 'clientId' | 'processId' | 'channel' | 'subject' | 'notes' | 'occurredAt'>>;
...
export function ManagementApp() {
  ...
  const [showClientForm, setShowClientForm] = useState(false); const [showProcessForm, setShowProcessForm] = useState(false); const [showDocumentForm, setShowDocumentForm] = useState(false); const [showInteractionForm, setShowInteractionForm] = useState(false); const [showTaskForm, setShowTaskForm] = useState(false); const [showFinanceForm, setShowFinanceForm] = useState(false);
  ...
  function updateDocument(documentId: string, patch: DocumentUpdate) { setDocuments((current) => current.map((document) => document.id === documentId ? { ...document, ...patch, updatedAt: new Date().toISOString() } : document)); }
  ...
  function updateInteraction(interactionId: string, patch: InteractionUpdate) { setInteractions((current) => current.map((interaction) => interaction.id === interactionId ? { ...interaction, ...patch } : interaction)); }
  ...
  else if (path === '/app/documentos') content = <DocumentsPage processes={processes} documents={documents} onCreateDocument={createDocument} onUpdateDocument={updateDocument} onToggleReceived={toggleDocumentReceived} showForm={showDocumentForm} onCloseForm={() => setShowDocumentForm(false)} />;
  else if (path === '/app/atendimentos') content = <InteractionsPage clients={clients} processes={processes} interactions={interactions} onCreateInteraction={createInteraction} onUpdateInteraction={updateInteraction} showForm={showInteractionForm} onCloseForm={() => setShowInteractionForm(false)} />;
