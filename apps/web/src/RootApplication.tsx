...
function normalizePath(pathname:string){const base=basePath();const path=base&&pathname.startsWith(base)?pathname.slice(base.length)||'/':pathname;return path.replace(/\/+$/,'')||'/'}
function withSharedSidebar(page:ReactNode){return <div className="crm-global-shell"><CrmSidebar/><div className="crm-global-page">{page}</div></div>}

export function RootApplication(){
 let path=normalizePath(window.location.pathname);
 if(path==='/crm/marketing/ia-criativa'){window.history.replaceState(null,'',`${basePath()}/crm/marketing`||'/crm/marketing');path='/crm/marketing'}
